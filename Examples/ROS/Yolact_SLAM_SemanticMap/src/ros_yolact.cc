...
public:
    ...
    ImageGrabber(ORB_SLAM2::System* pSLAM,cv::Mat kernel):mpSLAM(pSLAM),mkernel(kernel),nh("~")
    {
        //创建ROS的发布节点
        ...
        pub_odom= nh.advertise<nav_msgs::Odometry> ("Odometry", 50);
...
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD)
{
    ...
    std::pair<cv::Mat, cv::Mat> tcw_img = mpSLAM->TrackRGBD(cv_ptrRGB->image,cv_ptrD->image,mask,cv_ptrRGB->header.stamp.toSec(),mkernel,isKeyFrame);
    cv::Mat Tcw;
    cv::Mat rgb;
    tcw_img.first.copyTo(Tcw);
    tcw_img.second.copyTo(rgb);
    //cout<<1<<endl;
    //cv::imshow("Debug",rgb);
    //cv::waitKey(0);
    sensor_msgs::ImagePtr msgrgb = cv_bridge::CvImage(std_msgs::Header(), "bgr8", rgb).toImageMsg();
    //cout<<2<<endl;
    if (!Tcw.empty())
    {
        cv::Mat Twc =Tcw.inv();
