...
// 双目相机跟踪==================
 ...
 // rgbd相机跟踪===================  
    ...
    //===========================ros==========================
    std::pair<cv::Mat,cv::Mat> GrabImageRGBD(const cv::Mat &imRGB,
                          const cv::Mat &imD,
...
// 链表变量=============================
    ...
    void SetSegment(SegmentDynObject* segment);
...
// 由于 tracking 中需要用到 mLastFrame，这里检查并更新上一帧中被替换的 MapPoints
    ...
    bool Relocalization(int update = 0);           // 重定位  当前帧 和 所有关键帧 做匹配
    ...
    Frame mLastFrame;         // 上一帧 //, mLightLastFrame;
