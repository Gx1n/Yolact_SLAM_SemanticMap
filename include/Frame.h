/**
* This file is part of ORB-SLAM2.   加入了 掩码mask 
另外加入了一些 数据对象 是否为关键帧 彩色图 mask图等
* 普通帧 每一幅 图像都会生成 一个帧
* 
* 
*/
...
// 640 *480 的图像  分成 10 ×10个 64*48 的网格
...
public:
    ...
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, 
                  const cv::Mat &maskLeft, const cv::Mat &maskRight,  // 添加了 语义分割 mask
                  const double &timeStamp, 
                  ORBextractor* extractorLeft, ORBextractor* extractorRight,
                  ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, 
                  const float &bf, const float &thDepth);

    //  深度相机帧 灰度图 深度图 mask图 Constructor for RGB-D cameras.
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, 
                  const cv::Mat &imMask,                    // 添加了 语义分割 mask
                  const double &timeStamp, ORBextractor* extractor, 
                  ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, 
                  const float &bf, const float &thDepth);

    // 深度相机帧 灰度图 深度图 mask图 彩色图  Constructor for RGB-D cameras.
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, 
                  const cv::Mat &imMask, const cv::Mat &imRGB,      // mask图 & 彩色图
                  const double &timeStamp, ORBextractor* extractor, 
                  ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, 
                  const float &bf, const float &thDepth);

    // 单目相机帧 灰度图 Constructor for Monocular cameras.
    Frame(const cv::Mat &imGray, 
                  const cv::Mat &mask,     // 添加了 语义分割 mask
                  const double &timeStamp, ORBextractor* extractor,
                  ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef,
                  const float &bf, const float &thDepth);
    ...
    //  计算 特征 词典 描述向量 Compute Bag of Words representation.
    void ComputeBoW();

    // 设置相机位姿  Set the camera pose.
...
public:
   ...
   // 重定位 orb 词典 支持将 描述子 转成 用 单词线性表示的 向量
    ...
    // Depth Image (Included by Berta).  =========new ========
    cv::Mat mImDepth;  // 深度图
    bool mIsKeyFrame;  // 是否为 关键帧
    cv::Mat mImMask;   // mask掩码图
    cv::Mat mImGray;   // 灰度图
    cv::Mat mImRGB;    // RGB图
    ...
    std::vector<cv::KeyPoint> mvKeys, mvKeysRight;// 左图关键点  右图关键点
    std::vector<cv::KeyPoint> mvKeysUn;//校正后的关键点

    // Corresponding stereo coordinate and depth for each keypoint.
    // "Monocular" keypoints have a negative value.
    std::vector<float> mvuRight;// 左目特征点在 又目  匹配点的横坐标 差值
    ...
    std::vector<MapPoint*> mvpMapPoints;// 帧上关键点 关联到的 地图点
    ...
    vector<float> mvScaleFactors;// 关键点 所在 图像金字塔的 尺度因子
...
private:
    ...
    //Included by Berta ==============================new========
    void removeRow(cv::Mat &matIn, int row);// 去除某一行====

    // Rotation, translation and camera center
    cv::Mat mRcw;// 旋转矩阵 世界坐标系 到  相机坐标系 
    cv::Mat mtcw;// 平移向量
    cv::Mat mRwc;//旋转矩阵  相机坐标系 到  世界坐标系 
    cv::Mat mOw; //==mtwc 普通帧 相机 光行坐标（单目 左 相机）
};
