        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    //创建语义分割线程
    mpSegment = new SegmentDynObject();
    mptSegment = new thread(&ORB_SLAM2::SegmentDynObject::Run,mpSegment);
...
cv::Mat System::TrackRGBD(const cv::Mat &im, 
                          ...
                          cv::Mat &mask,           // 语义分割mask 输入
                          const double &timestamp,cv::Mat kernel)
{
    if(mSensor!=RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
...
//=======================================ros==============================================
std::pair<cv::Mat,cv::Mat> System::TrackRGBD(const cv::Mat &im,
                          const cv::Mat &depthmap,
                          cv::Mat &mask,           // 语义分割mask 输入
                          const double &timestamp,cv::Mat kernel,bool& isKeyframe)
{
    if(mSensor!=RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    // 跟踪接口变化===================加入语义分割mask 输入
    mpTracker->GetImg(im);
    std::pair<cv::Mat, cv::Mat> tcw_img = mpTracker->GrabImageRGBD(im,depthmap,
                                           mask,           // 语义分割mask 输入
                                           timestamp,kernel,isKeyframe);
