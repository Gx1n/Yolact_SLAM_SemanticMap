    ...
    def prep_display(self, dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=1.0, fps_str=''):
        ...
        if (num_dets_to_consider):
            ...
            if(person):
                ...
                mask1 = np.ones((h, w ,3),dtype =np.float32)#必须使float32 ！！
                mask1 = mask1 - img_mask_dil
                result = mask1 * img_crop
                return result
            else:
                return img_crop
            # result =[]
...
if __name__ == '__main__':
    #tmp_str='''
    img_path="/home/x1/catkin_ws/src/Yolact_SLAM/src/yolact/2.png"
    img=cv2.imread(img_path,cv2.IMREAD_UNCHANGED)
    #mask=np.ones((480,640,3))
    masknet=YOLACT()
    result=masknet.GetDynSeg(img)
    #print(result.dtype)
