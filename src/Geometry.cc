...
Geometry::Geometry()
{
    vAllPixels = cv::Mat(640*480,2,CV_32F);// 两列  存储行列id
    int m(0);
    for (int i(0); i < 640; i++){
        for (int j(0); j < 480; j++){
...
// 获取最大最小 角度差 和 距离差 再归一化===============
    ...
    for (int i(0); i < mnRefFrames; i++)
    {
        ...
        vRefFrames.push_back(mDB.mvDataBase[ind]);// 上面是降序排列，这里从前面取，是相似性差值最大的几个???
    }
    return(vRefFrames);
}
...
// 步骤1：计算参考帧关键点 的世界坐标系坐标=============================================================
	...
	// vconcat（B,C，A）; // 等同于A=[B ;C]
        cv::vconcat(vMPRefFrame,matInvDepthRefFrame.t(),vMPRefFrame);//vMPRefFrame 维度为 4×k    (X/Z，Y/Z，1，1/Z)， 如果要是乘以Z的话就变成(X,Y,Z,1),3D坐标的齐次形式
        cv::Mat vMPw = refFrame.mTcw.inv() * vMPRefFrame;// 相机坐标系转换到 世界坐标下 --->   维度还是   4×k, (X/Z，Y/Z，1，1/Z), 不过值变了...
...
// 步骤4.1 保留 反投影后深度值小于 7m的点==================================================== 
        // 中间变量 
        cv::Mat _vMPCurrentFrame = cv::Mat(vMPCurrentFrame.size(),CV_32F);    //  当前坐标系下3d坐标齐次形式     (X,Y,Z,1)    4×M
        cv::Mat _vAllMatRefFrame = cv::Mat(vAllMatRefFrame.size(),CV_32F);          //  参考帧像素坐标齐次形式 (u,v,1)      M×3
        cv::Mat _vLabels = cv::Mat(vLabels.size(),CV_32F);                                                //  所属 参考帧id                           M×1
        cv::Mat __vAllDepthRefFrame = cv::Mat(vAllDepthRefFrame.size(),CV_32F);// 参考帧上深度                            M×1
        int h(0);
        ...
        for (int i(0); i < matProjDepth.cols; i++)//  关键帧点 反投影后的 Z,深度值====1×M
        {
            ...
            {
                __matProjDepth.at<float>(0,h) = matProjDepth.at<float>(0,i);// 关键帧点 反投影后在当前帧下的 Z,深度值

                _vMPCurrentFrame.at<float>(0,h) = vMPCurrentFrame.at<float>(0,i);// 当前坐标系下3d坐标齐次形式     (X,Y,Z,1)  
                ...
                _vAllMatRefFrame.at<float>(h,0) = vAllMatRefFrame.at<float>(i,0);//  参考帧像素坐标齐次形式 (u,v,1)  
                ...
                __vAllDepthRefFrame.at<float>(h,0) = vAllDepthRefFrame.at<float>(i,0);//  参考帧上深度   
        ...
        vMPCurrentFrame = _vMPCurrentFrame.colRange(0,h);//  当前坐标系下3d坐标齐次形式     (X,Y,Z,1)    4×h
...
void Geometry::FillRGBD(const ORB_SLAM2::Frame &currentFrame,
                                                   cv::Mat &mask,
                                                   cv::Mat &imGray,
                                                   cv::Mat &imDepth)
{
