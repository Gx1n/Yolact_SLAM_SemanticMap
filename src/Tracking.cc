...
新 idear:
    ...
    深度     依靠 语义分割信息 和 深度图 几何学 动静mask 剔除 最大可能的 运动物体
            先轻两级别跟踪，获取当前帧位姿，最近5个参考帧关键点投影到本帧
            保留角度小的，深度值合理的，能够投影到本帧图像上的
            若投影后的深度 和 本帧深度图 测量深度差别较大则认为是动态点
            
            这里考虑使用 光流法来判断是否是 动态点======
            
            动/静mask 和 语义mask 结合
            
            剔除动点和 语义点 后再次使用 原来的方法进行最后的跟踪==========
...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc,
                   ...
                   MapDrawer *pMapDrawer,
                   Map *pMap,
                   shared_ptr<PointCloudMapping> pPointCloud, //点云
                   KeyFrameDatabase* pKFDB,
                   const string &strSettingPath,
                   const int sensor):
        mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false),
...
void Tracking::SetSegment(SegmentDynObject* segment)
{
    mpSegment=segment;
}
                                ...
                                const double &timestamp,cv::Mat kernel)
{
    ...
    cout << "wait for new segment img time  =" << ttrack1*1000 << endl;
    // Segment out the images
    cv::Mat maskRCNN = mpSegment->mImgSegmentmask;
    cv::Mat maskRCNNdil = maskRCNN.clone();
    cv::dilate(maskRCNN,maskRCNNdil, kernel);
    mask = mask - maskRCNNdil;
    cv::Mat imMask = mask;// 语义分割 mask===================
    mImRGB = mpSegment->mImgSegmented;
...
//=====================================ros===================================
std::pair<cv::Mat,cv::Mat> Tracking:: GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD,
                                 cv::Mat &mask,
                                 const double &timestamp,cv::Mat kernel,bool& isKeyframe)
{
    ...
    std::pair<cv::Mat, cv::Mat> tcw_img = std::make_pair(mCurrentFrame.mTcw.clone(),mImRGB);
...
{
    unique_lock<mutex> lock(mpSegment->mMutexNewImgSegment);
    if(mbNewSegImgFlag)
    {
